...
class GroupsMemoryCard{
	constructor(groups){
		...
		this.updateName();
		this.gui = new GUI(this.memcardReader, container, this);
		this.updateMembers();
		this.toMemoryCard();
	}
	updateMembers(){
	...
	toMemoryCard(){
		...
		for(let member of this.members){
			...
			if(index != -1){
				this.memcardReader.saveSlotArray[index].setMember(member);
				this.gui.updateSlotButtons();
			}
		}
	}
	toHalfMemoryCard(groupIndex){
		let index;
		this.members.splice(this.members.length/2*groupIndex, this.members.length/2);
		for(let member of this.members){
			index = this.memcardReader.addDigimonSave();
			if(index != -1){
				this.memcardReader.saveSlotArray[index].setMember(member);
	...
	downloadHalfMemoryCard(index){
		this.updateMembers();
		this.updateName();
		this.memcardReader.wipeMemoryCard();
		this.toHalfMemoryCard(index);
		let link = document.createElement('a');
		link.style.display = 'none';
		document.body.appendChild(link);
		link.href = URL.createObjectURL(this.memcardReader.getDigimonSavesBlob());
		link.download = `${new Date().toISOString().slice(0, 16).replaceAll("-", "").replaceAll("T", "").replaceAll(":", "")}_${this.groups[index].name}.mcr`;
		link.click();
		document.body.removeChild(link);
		this.updateMemoryCard();
	}
	downloadFullMemoryCard(){
		let link = document.createElement('a');
		link.style.display = 'none';
		document.body.appendChild(link);
		link.href = URL.createObjectURL(this.memcardReader.getDigimonSavesBlob());
		link.download = `${new Date().toISOString().slice(0, 16).replaceAll("-", "").replaceAll("T", "").replaceAll(":", "")}_${this.name}.mcr`;
		link.click();
		document.body.removeChild(link);
	}
}
...
class Order{
	...
	swapParticipants(sourceParticipant, targetParticipant){
		let sourceParticipantIndex = order.groups[sourceParticipant.groupName].members.indexOf(sourceParticipant);
		let targetParticipantIndex = order.groups[targetParticipant.groupName].members.indexOf(targetParticipant);
		let temp = sourceParticipant.groupName;
		sourceParticipant.setGroupName(targetParticipant.groupName);
		targetParticipant.setGroupName(temp);
		order.groups[sourceParticipant.groupName].members.splice(targetParticipantIndex, 1, sourceParticipant);
		order.groups[targetParticipant.groupName].members.splice(sourceParticipantIndex, 1, targetParticipant);
		updateGroupsMemoryCards();
	}
	getKlixuzSpreadSheetData(){
		let retArr = [];
...
const possibleGroupSizes = [4, 5, 6, 7, 8, 9, 10];
const possibleTournamentSizes = [4, 5, 6, 7, 8, 10, 12, 15, 16, 18, 20, 24, 25, 28, 30, 32, 35, 36, 40, 42, 48, 128];
