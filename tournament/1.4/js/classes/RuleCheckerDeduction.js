...
compensationPerLesserMoveOnList = 100;
compensationPerRestrictedMove = 100;
compensationPerSlowMove = 50;
compensationPerBadAcuracyMove = 50;
*/
const deductionMoves = {
	"lesserMoves": [[], 100],
	"restricetdMoves": [[], 100],
	"slowMoves": [[], 50],
	"badAccuracyMoves": [[], 50]
...
const finisherPercent = 10;
const maxCompensationPercent = 20;
...
function filterDeductionMoves(moves, type){
	let foundDeductionMoves = [];
    for(let move of moves){
		if(deductionMoves[type][0].indexOf(move.name) != -1){
			foundDeductionMoves.push(move);
        }
	}
	return foundDeductionMoves;
}
...
for(let moveRule of moveRules){
	deductionMoves["restricetdMoves"][0].push(moveRule.name);
}
...
class Rules{
    ...
    reset(){
        ...
        this.compensateTotalStatsBy = 0;
        this.compensateTotalStatsByReason = [];
        this.moves = [];
        this.bannedMoves = [];
	...
	checkCompensations(digiStats){
		if(this.moves.length > maxMovesetLength){
			return;
		}
		if(digiStats.finisher == "None"){
			this.compensateTotalStatsBy = Math.floor(maximumCombinedStats * finisherPercent / 100);
		}
	}
    checkRules(digi){
        this.reset();
		...
		this.checkCompensations(digiStats);
...
/*
MoveEntries; //number of moves available to Digimon
maxMoveset = 6; //the amount for Digimon's available moves
maxRequieredUnrestrictedMoves = 3; //the maximum available Moves which are not resticted
restrictedMoves; //number of Moves that are restricted via rules | ARRAY type?
BadMoves; //Number of Moves that are considered bad in one way or another | ARRAY type?
...
Deduction:
decuctionPerMoveOverEntryLimit = 100;
decuctionPerFastMove = 0;

Finisher Compensation:
compensationOnEmptyFinisher = 10% of Tournament Specific stat Points // 11th Zathournament [4000] -> +400 points

Extra stats distribution CAP?
extraStatDistributionCap = 20% of Tournament Specific stat Points // 11th Zathournament [4000] = max -> +800 points



Code section:
If (MoveEntries <= maxMoveset)
{
//normal obtainable Digimon have at least 7 Moves available to them
((maxMoveset +1) - MoveEntries) * compensationPerLesserMoveOnList;
//Example: (( 6 + 1 ) - MoveEntries) * 100 )
}

If ((MoveEntries - restrictedMoves) <= 3)
{
If (ignoreBadMoves = ture)
{
//multiplying the compensation by the number of restricted moves (dependent on the available Moves from Digimon)
restrictedMoves * compensationPerRestrictedMove;
}
else ((MoveEntries - (restrictedMoves + BadMoves)) <= 3)
{
restrictedMoves * compensationPerRestrictedMove;
BadMoves * compensationPerSlowMove;
BadMoves * compensationPerBadAcuracyMove;
}
}
*/
